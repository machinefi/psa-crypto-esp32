...
class PsaHashCompute : public ::testing::Test
{
  protected:
	...
	const uint8_t expected_hash[32] = {0x9f, 0x86, 0xd0, 0x81, 0x88, 0x4c, 0x7d, 0x65,
...
TEST_F(PsaHashCompute, InvalidHashAlgorithm)
{
	...
	psa_crypto_init();
	psa_status_t status =
		psa_hash_compute(PSA_ALG_MD5, msg, sizeof(msg), hash, sizeof(hash), &hash_length);
...
TEST_F(PsaHashCompute, NotAHashAlgorithm)
{
	...
	size_t hash_length = 0;
	psa_crypto_init();
	psa_status_t status = psa_hash_compute(PSA_ALG_HMAC(PSA_ALG_SHA_256), msg, sizeof(msg), hash,
...
TEST_F(PsaHashCompute, BufferTooSmall)
{
	...
	psa_crypto_init();
	psa_status_t status =
		psa_hash_compute(PSA_ALG_SHA_256, msg, sizeof(msg), hash, sizeof(hash) - 1, &hash_length);
